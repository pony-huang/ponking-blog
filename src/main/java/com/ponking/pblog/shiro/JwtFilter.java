...
//                try {
//                    this.requestDispatcher(request, response);
...
//                return false;
    ...
    private boolean refreshToken(ServletRequest request, ServletResponse response) {
        ...
        String token = this.getAuthzHeader(request);
        // 获取当前Token的帐号信息
        String account = JwtUtil.getClaim(token, AuthConstants.ACCOUNT);
        // 判断Redis中RefreshToken是否存在
        if (RedisUtil.exists(AuthConstants.PREFIX_SHIRO_REFRESH_TOKEN + account)) {
            ...
            if (JwtUtil.getClaim(token, AuthConstants.CURRENT_TIME_MILLIS).equals(currentTimeMillisRedis)) {
                ...
                RedisUtil.setObject(AuthConstants.PREFIX_SHIRO_REFRESH_TOKEN + account, currentTimeMillis, AuthConstants.REFRESH_TOKEN_EXPIRE_TIME, TimeUnit.SECONDS);
